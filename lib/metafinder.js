...
function getDocumentList(textQuery,tagArray,notTagArray,limit,offset) {
    ...
    tagArray.forEach(element => {
        tagQuery = tagQuery + '{{ ?s tridoc:tag ?tag' + element + ' . ?tag' + element + ' tridoc:label \"' + element + '\" . } UNION { ?s tridoc:tag ?ptag' + element + ' . ?ptag' + element + ' tridoc:parameterizableTag ?atag' + element + ' . ?atag' + element + ' tridoc:label \"' + element + '\" . }} . \n'
    });
    let notTagQuery = "";
    notTagArray.forEach(element => {
        notTagQuery = notTagQuery + 'FILTER NOT EXISTS {{ ?s tridoc:tag ?ntag' + element + ' . ?ntag' + element + ' tridoc:label \"' + element + '\" . } UNION { ?s tridoc:tag ?nptag' + element + ' . ?nptag' + element + ' tridoc:parameterizableTag ?natag' + element + ' . ?natag' + element + ' tridoc:label \"' + element + '\" . }} . \n'
    });
...
function getDocumentNumber(textQuery,tagArray) {
    let tagQuery = "";
    tagArray.forEach(element => {
        tagQuery = tagQuery + '{{ ?s tridoc:tag ?tag . ?tag tridoc:label \"' + element + '\" . } UNION { ?s tridoc:tag ?ptag . ?ptag tridoc:parameterizableTag ?atag . ?atag tridoc:label \"' + element + '\" . }} . \n'
    });
    return fetch("http://fuseki:3030/3DOC/query", {
        method: "POST",
        headers: {
