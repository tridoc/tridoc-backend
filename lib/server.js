...
async function start() {
    ...
    server.route({
        ...
        handler: function (request, h) {
            ...
            let notTagArray = (request.query.nottag ? makeArray(request.query.nottag) : [])
            return metaFinder.getDocumentNumber(request.query.text, tagArray, notTagArray);
        }
    });
        ...
        config: {
            handler: (request, h) => {
                ...
                return pdfProcessor.getText(request.payload.path).then(text => {
                  if (text.length < 4) {
                    const sandwich = spawnSync( 'pdfsandwich', [ request.payload.path ] );
                    if(sandwich.error) {
                    ...
                    return metaStorer.storeDocument(id, text, request.query.date.match(datecheck) ? request.query.date : undefined).then(() => {
    ...
    server.route({
        ...
        handler: function (request, h) {
            ...
            let tagArray = (request.query.tag ? makeArray(request.query.tag) : []);
            let notTagArray = (request.query.nottag ? makeArray(request.query.nottag) : [])
            return metaFinder.getDocumentList(request.query.text, tagArray, notTagArray, request.query.limit, request.query.offset);
        }
    });
