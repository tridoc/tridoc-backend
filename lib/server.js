...
async function start() {
    ...
    server.route({
        ...
        handler: function (request, h) {
            log_info(request);
            return metaFinder.getDocumentNumber(processParams(request.query));
        }
    });
    ...
    server.route({
        ...
        handler: function (request, h) {
            log_info(request);
            return metaFinder.getDocumentList(processParams(request.query));
        }
    });
    ...
    server.route({
        ...
        config: {
            handler: (request, h) => {
                console.log(request);
                var label = decodeURIComponent(request.params.label);
                const arg = processParams(request.query)
                arg.nottags = []
                arg.tags = makeArray(label)
