...
export function hashToPath(hash: string): { dir: string; fullPath: string } {
  ...
  const dir = `./blobs/ipfs/${hash.slice(0, 4)}/${hash.slice(4, 8)}/${hash.slice(8, 16)}`;
 ...
 */
export function hashToThumbnailPath(hash: string): { dir: string; fullPath: string } {
  // Store in blobs/thumbs/ subdirectory using same structure as blobs
  const dir = `./blobs/thumbs/${hash.slice(0, 4)}/${hash.slice(4, 8)}/${hash.slice(8, 16)}`;
